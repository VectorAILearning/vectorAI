...
export default function QuestionContent({ questionContent }) {
    ...
    return (
        <div>
            ...
            <ul className="ml-6">
                {questionContent.options?.map((opt: string, i: number) => (
                    <li key={i}>
                        ...
                        <input disabled={isSolution} type="radio" name="question_options" id="" value={opt} onChange={() => 
                            setSelectedOptions(opt)
                            }/>
                        <label className='ml-[10px]' htmlFor="">{opt}</label>
            ...
            <div>
                <button onClick={handleSubmit} disabled={isSolution} className="btn btn-primary">Ответить</button>
                {
                    SolutionError && <div>
