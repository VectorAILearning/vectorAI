...
async def generate_block_content(ctx, block_id: str, lesson_id: str, **kwargs):
    async with uow_context() as uow:
        ...
        if not block or not lesson:
            log.error(
                f"Block or lesson not found: block_id={block_id}, lesson_id={lesson_id}"
            )
            return
        log.debug(
            f"Начинаю генерацию контента для блока: block_id={block_id}, type={block.type}, description={block.description}, goal={block.goal}"
        )
        try:
            await ContentService(uow).generate_content_by_block(block)
            log.debug(
                f"Контент успешно сгенерирован и сохранён для блока: block_id={block_id}"
            )
        except Exception as e:
            log.exception(
                f"Ошибка при генерации контента для блока: block_id={block_id}, error={e}"
            )
            return
        ...
        if next_block:
            log.info(
                f"Следующий блок найден: id={next_block.id}, type={next_block.type}, position={next_block.position}"
            )
            ...
            log.info(
                f"Задача на генерацию контента для блока {next_block.id} поставлена в очередь"
            )
        else:
            log.info(
                f"Следующего блока не найдено для lesson_id={lesson_id}, position={block.position + 1}"
            )
