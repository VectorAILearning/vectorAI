    ...
    try:
        ...
        async with uow_context() as uow:
            ...
            user_pref = await AuditDialogService(
                uow
            ).create_user_preference_by_audit_history(audit_history, sid=sid, user_id=user_id)
        ...
        await push_and_publish(_msg("bot", "Предпочтение пользователя сгенерировано"), sid)

        if generate_tasks_context["task_type"] == TaskTypeEnum.generate_course.value:
            if generate_tasks_context["params"].get("deep") != GenerateDeepEnum.user_summary.value:
        ...
        await push_and_publish(
            _msg("bot", f"Произошла ошибка при генерации предпочтения пользователя: {e}", "error"),
            sid,
        )
        await push_and_publish(
            _msg("system", "Ошибка при генерации предпочтения пользователя", "user_preference_generation_error"),
            sid,
        )
        if sid:
