...
— Все тексты, только на русском языке, строго в формате JSON.
...
— Для каждого блока указывай:
  ...
  - outline       (строка с 1–3 тезисами, перечисленными через ;")
...
— Все тексты только на русском языке (кроме кода), строго в формате JSON, как в примере ниже.
...
— Если тема урока связана с программированием, синтаксисом, алгоритмами, обязательно включай хотя бы один блок типа code 
с большим или исполняемым фрагментом кода и кратким объяснением, если это актуально для урока.
- Если есть практика, то делай к ней подводку (блоки text в котором частично раскрывается решение), чтобы ученик понял, как её решать.
...
[
  {{
    ...
    "outline": "Что такое условие; Синтаксис if/else; Пример с переменной age;"
  }},
  {{
    "type": "code",
    "position": 2,
